...
| `SMOOSENSE_LOCAL_FOLDER_PREFIX` | Controls local folder access. Unset: auto-detect (allowed on localhost, denied elsewhere). `""`: deny all. `*`: allow all. `/prefix/`: allow only paths starting with the prefix. | unset |
